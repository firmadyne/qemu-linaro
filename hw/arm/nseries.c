...
static void n900_init(MachineState *machine)
{
    MemoryRegion *sysmem = get_system_memory();
...
#error MAX_SERIAL_PORTS must be at least 3!
    ...
    if (dsd) {
        omap3_mmc_attach(s->cpu->omap3_mmc[1], blk_by_legacy_dinfo(dsd),
                         0, 1);
    }
